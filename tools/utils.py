...
def make_eval_report(depth_gt: np.array, depth_map: np.array, cam_calib: dict) -> str:
    ...
    result_rev = np.zeros((cam_calib["size"]["height"], cam_calib["size"]["width"]), dtype=np.float32)
    new_gt = []
    new_pred = []
    min_depth=0
    max_depth=80
    ...
    print(dict(a1=a1, a2=a2, a3=a3, abs_rel=abs_rel, rmse=rmse, log_10=log_10, rmse_log=rmse_log, silog=silog, sq_rel=sq_rel))
    report = ""
