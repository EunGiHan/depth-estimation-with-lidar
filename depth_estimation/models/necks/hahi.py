    ...
    def __init__(
        ...
        for in_channel, out_channel in zip(in_channels, out_channels):
            self.lateral_convs.append(
                ConvModule(
                    in_channel, out_channel, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg
                )
            )
        ...
        for in_channel, out_channel in zip(out_channels[1:], out_channels[1:]):
            self.trans_fusion.append(
                ConvModule(
                    out_channel + self.embedding_dim,
                    out_channel,
                    kernel_size=3,
                    padding=1,
                    stride=1,
        ...
        self.conv_fusion = nn.Sequential(
            ConvModule(
                in_channels[0] + self.embedding_dim,
                out_channels[0],
                kernel_size=3,
                padding=1,
                stride=1,
        ...
        self.multi_att = MultiScaleDeformableAttention(
            embed_dims=self.embedding_dim,
            num_levels=4,
            num_heads=8,
