...
class PatchMerging(BaseModule):
    ...
    def __init__(self,
        ...
        sample_dim = stride ** 2* in_channels # norm_layer(2 * dim)
...
class WindowMSA(BaseModule):
    ...
    Args:
        ...
        init_cfg (dict | None, optional): The Config for initialization.
            Default: None.
    """
    # MOD: qk_scale 삭제, pretrained_window_size 추가
    def __init__(self,
                 embed_dims,
                 num_heads,
                 window_size,
        ...
        # MOD: self.pretrained_window_size 추가
        self.pretrained_window_size = pretrained_window_size
        self.num_heads = num_heads
        # MOD: scale 삭제 -> log scale로
        # head_embed_dims = embed_dims // num_heads
        # self.scale = qk_scale or head_embed_dims**-0.5
        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((num_heads, 1, 1), device="cuda:0")), requires_grad=True)

        # MOD: relative_position_bias table 변경
    ...
    def forward(self, x, mask=None):
        ...
        # MOD: add cosine attention
        # q = q * self.scale
        # attn = (q @ k.transpose(-2, -1))
        
        # cosine attention
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01, device="cuda:0"))).exp()
        attn = attn * logit_scale

        # ------------------------------------------------------>
        # MOD: relative_position_bias 수정
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            if i < num_layers - 1:
                ...
                downsample = None
                # MOD: 마지막 레이어에 대해 이렇게 처리해줘야 돌아간다.. 이유는 아직 모르겠다.
        ...
        # MOD: init_respostnorm 추가
        # 근데 pretrained 불러오면 어차피 init이 필요 없을 수도 있음
